...
public class Board implements IBoard {
    ...
    private final IFirewall firewall;
    private final int row, col, k;
    private final int full;
    private final int[] fullness = {0};

    public Board(int row, int col, int k) {
        this.board = new ECell[row][col];

        for (ECell[] erow : board) {
            Arrays.fill(erow, ECell.E);
        }
        ...
        this.firewall = new Firewall(board, row, col, k, full, fullness);
    }
    ...
    @Override
    public boolean isRight(IMove move) {
        int col = move.getCol();
    ...
    public EResult makeMove(IMove move) {
        board[getRow(move.getCol())][move.getCol()] = move.getPLayer();

        EResult res = getResult(move);
        fullness[0]++;
