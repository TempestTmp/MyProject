# MyProject
This repository contains some of my projects.

connectFour - This is a "Connect Four" game written in Java

checkmate_with_a_rook_and_a_king - This is a program written in java that checks the opponent's king with the help of its king and queen.

myScanner - This program is written in Java and improves the built-in scanning method, speeding it up.

NumberByIEEE754 - This is an emulation of the IEEE-754 protocol in Python programming language

MyXorshiftGen - Implementation of a pseudorandom number generator in the c++ programming language and comparison of the speed of multithreaded calculations

ApplicationSimpleСiphers - This is a school project written in python. The application allows you to delve into the study of cryptography in more detail.

Возрождение_2_0 - This application is an advertisement written as part of the Hackathon program from Polytech. The award-winning audience award. Lets you learn more about the Arctic

